...
# Make pseudo-SE sam-files, Homer Tag Directories and extract QC from TD:
...
# Make tagdirectory and capture output
	...
	makeTagDirectory ${SAMFILE/.sam.dedup.sam.dedup_SE.sam/.dedup.TD\/} $SAMFILE -format sam -keepAll -fragLength 220 -genome mm10 -checkGC -tbp 1 2> $SAMFILE.homer 
...
# As the sequencing deepth of SM3640 is much higher than all other samples, this TD is downsampled to 20 mill reads by running the R-script 'Rscript Downsample_Maria_updated.R' (see seperate script)
# make TD from the generated tsv-file
	makeTagDirectory SM3640_downsampled.TD -t SM3640_star.Aligned.out.primary.dedup.TD_Downsampled_to_2e+07.tags.tsv
...
# Identify peaks and calculate FRIP (fraction in peaks):
	#!/bin/bash
	for TAGDIRECTORY in $(ls -d SM*.TD); do
	echo "Doing Homer FindPeaks on " $TAGDIRECTORY
	findPeaks $TAGDIRECTORY/ -style histone 1> ${TAGDIRECTORY/.TD/.peaks} 
	done
...
#____________________________________________________________
# Annotate TD to the PPARgWT peaks extended to a total of 3000 bp (+/-1500 bp from center of peak). 
	annotatePeaks.pl PPARgWT_peaks_factor_lbg20kb_blacklisted.bed mm10 -noadj -size -1500,1500 -d *'H3K27ac'.TD > TagCounts_WTpeaksExt_H3K27ac_mm10.txt
# Annotate TD to the PPARgWT peaks extended to a total of 3000 bp (+/-1500 bp from center of peak), in bins of 50 bp. 	
	annotatePeaks.pl HA_peaks_35.bed  mm10 -nogene -noann -size -1500,1500 -hist 50 -ghist -d *.TD > TagCounts_WTpeaksTH35Ext_H3K27ac_hist_adj_mm10.txt